...
function removeClientRequest(url, client_id) {
  ...
  httpRequest.onreadystatechange = () => {
    if (httpRequest.readyState === XMLHttpRequest.DONE) {
      if (httpRequest.status === 200) {
        ...
        makeToast(`Client Removed`, `The client "${response.client_name}" is removed.`);


        document.getElementById(`clientRow${client_id}`).remove();
      } else {
        makeToast(`Client NOT Removed`, `There was an error removing the client.`)
      }
    }
  }
  ...
  httpRequest.send(`client_id=${encodeURIComponent(client_id)}`);
}

function removeClient(client_id) {
  removeClientRequest('client_action.php', client_id);
}

function editClient(client_id) {
  window.location.href = `client_edit.php?client_id=${client_id}`;
}
