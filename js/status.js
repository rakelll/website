function makeToast(title, body_text) {
  ...
  if (!(document.getElementById("requestStatusToasts"))) {
    const newToastContainer = document.createElement("div");
    newToastContainer.classList.add("toast-container", "top-0", "end-0", "p-2", "position-fixed");
    newToastContainer.setAttribute("id", "requestStatusToasts");

    document.body.appendChild(newToastContainer);
  }

  const newToastContainer = document.getElementById("requestStatusToasts");
...
function removeStatusRequest(url, status_id) {
  ...
  httpRequest.onreadystatechange = () => {
    if (httpRequest.readyState === XMLHttpRequest.DONE) {
      if (httpRequest.status === 200) {
        ...
        makeToast(`Status Removed`, `The Status "${response.status_name}" is removed.`);


        document.getElementById(`statusRow${status_id}`).remove();
      } else {
        makeToast(`Status NOT Removed`, `There was an error removing the Status "${response.status_name}".`)
      }
    }
  }
  ...
  httpRequest.send(`status_id=${encodeURIComponent(status_id)}`);
}

function removeStatus(status_id) {
  removeStatusRequest('status_action.php', status_id);
}

function changeStatusRequest(url, status_id, new_Status) {
  let httpRequest = new XMLHttpRequest();
  ...
  httpRequest.onreadystatechange = () => {
    if (httpRequest.readyState === XMLHttpRequest.DONE) {
      if (httpRequest.status === 200) {
        ...
        makeToast(`Status Changed`, `The Status "${response.last_status}" is changed to "${response.status_name}".`);
      } else {
        makeToast(`Status NOT Changed`, `There was an error changing tthe status.`)
      }
    }
  };
  ...
  httpRequest.send(`status_id=${encodeURIComponent(status_id)}&status_change=${encodeURIComponent(new_Status)}`);
}

function editStatus(status_id) {
  const editButton = document.getElementById(`statusButton${status_id}`);
  const editSpan = document.getElementById(`statusSpan${status_id}`);
  ...
  editButton.setAttribute("onclick", `saveStatusChange(${status_id})`);

  editSpan.setAttribute("contenteditable", "true");
  editSpan.style.border = "1px solid gray";
}

function saveStatusChange(status_id) {
  const statusSpan = document.getElementById(`statusSpan${status_id}`).textContent.trim();

  changeStatusRequest('status_action.php', status_id, statusSpan);
  ...
  editButton.setAttribute("onclick", `editStatus(${status_id})`);
